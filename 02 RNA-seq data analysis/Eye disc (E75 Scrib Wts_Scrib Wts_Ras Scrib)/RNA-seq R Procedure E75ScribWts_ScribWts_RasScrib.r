...
library(ggrepel)
library(patchwork)
setwd("G:\\E75\\RNA_seq part\\04Expression\\E75ScribWts_ScribWts_RasScrib")                    #设置工作目录 EyeDisc 替换为 WingDisc
setwd("L:\\E75\\RNA_seq part\\04Expression\\E75ScribWts_ScribWts_RasScrib")   
...
rt=read.table("E75ScribWts_ScribWts_RasScrib.tsv",header = T,sep = "\t")             #改成自己的文件名
rownames(rt)=rt[,1]
exp=rt[,2:ncol(rt)]
data = exp
...
group_info = c(rep("Scrib_Wts",3), rep("Ras_Scrib",3), rep("E75_Scrib_Wts",3))
...
dge.list.res <- exactTest(dge.list.obj, pair = c("Scrib_Wts","Ras_Scrib"))
...
#mart <- useDataset("dmelanogaster_gene_ensembl", useMart("ensembl",host = "https://asia.ensembl.org/"))
mart <- useEnsembl(biomart = "ensembl", mirror = "asia")
mart=useDataset("dmelanogaster_gene_ensembl", mart = mart)
...
ggsave(filename = "Part2_VolcanoPlotNewUnlabel.pdf",plot = p, width = 15,height = 12, units = "cm", dpi = 600)
...
p6
ggsave(filename = "Part2_VolcanoPlotNew.pdf",plot = p6, width = 15,height = 12, units = "cm", dpi = 600)


#需要label的gene
genes = c("Rdl","mGluR","deltaTry","Dh44-R2","thetaTry","etaTry","CapaR","CCHa2-R","5-HT1A","Lcch3","betaTry",
                "alphaTry","epsilonTry","Tspo","Nmdar1","gammaTry","CG30031","GABA-B-R3","CCAP-R","mAChR-A",
				"Lgr1","TkR99D","mAChR-B","Dh44-R1","GluRIA","PK2-R2","Dop1R1","Eip75B","Eip78C","Eip63F-1","Eip93F")

genes = c("GV1","Cht10","ImpE3","Egfr","thetaTry","sut2","Spn100A","CG14880","bab2","CG4766","CG17919",
                "Blimp−1","emp","flap","CG7896","lncRNA:CR40469","CG15784","nw","Lim1","ap",
				"CG5535","CG12964","Dgp−1","drd","Aldh","CG18278","aos","Eip75B","CG6830","Gdh","CG44325","Gapdh2","spz3")			
...
p2 <- p +
  ...
  geom_text_repel(data = input_label_up,
                  ...
                  segment.color = 'black', #线段颜色
                  segment.alpha = 0.8, #线段不透明度
                  nudge_x = 12 - input_label_up$logFC, #标签x轴起始位置调整
                  direction = "y", #按y轴调整标签位置方向，若想水平对齐则为x
                  hjust = 1 #对齐标签：0右对齐，1左对齐，0.5居中
  )
p2
...
p3
ggsave(filename = "Part2_VolcanoPlotNewSpeciallabel.pdf",plot = p3, width = 15,height = 12, units = "cm", dpi = 600)
...
#ENTREZID 就是 ncbi-geneid
ekegg <- enrichKEGG(gene = gene.kegg$'ncbi-geneid', 
                    organism = "dme", 
                    keyType = "ncbi-geneid",
                    pvalueCutoff  = 0.05,
                    pAdjustMethod = "BH",  
...
write.table(keggres, file = "Part2_KEGG Results.txt",sep = "\t")

#KEGG 输出结果的ID转化：https://mp.weixin.qq.com/s/p7hLw07zC40LTVOFD5oAng
# 提取富集结果
kegg <- ekegg@result
...
entrezID2geneSymbol <- function(entrezID){
  ...
  SYMBOL <- bitr(entrezID,"ENTREZID","SYMBOL",org.Dm.eg.db)[,"SYMBOL"]
  # 再SYMBOL合并为字符串，并返回结果
...
write.table(kegg, file = "Part2_KEGG Results_genename.txt",sep = "\t")
...
pheatmap_list_count_new <- pheatmap_list_count_new[,c("E751A","E752A","E753A","WT1A","WT2A","WT3A")]
...
ggsave(filename = "Part3_Heatmap_Notch.pdf",plot = heatmap, width = 14,height = 12, units = "cm", dpi = 600)
...
ggsave(filename = "Part3_Heatmap_Notch_All.pdf",plot = heatmap, width = 14,height = 54, units = "cm", dpi = 600)
